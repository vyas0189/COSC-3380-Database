...
router.put('/update', auth, async (req, res) => {
    try {
        ...
        const addressDB = await db.query('UPDATE address SET address_name = $1, address2_name = $2, city = $3, state = $4, zip = $5 WHERE address_id = $6 RETURNING *',
            [address, address2, city, state, zip, user.rows[0].patient_address]);

        console.log('Updated Address', addressDB.rows);
        ...
        console.log('Updated Patient', patient.rows);
