...
const {
	updateDoctor, updateDiagnosis, orderTest, validate,
} = require('../validation');
...
router.post('/order/test', doc, async (req, res) => {
	try {
		...
		const doctor = await db.query('SELECT * FROM doctor WHERE doctor_user = $1', [userID]);
		...
		await db.query('INSERT INTO test(test_scan, test_physical, test_blood, test_office, test_doctor, test_patient, test_diagnosis) VALUES($1, $2, $3, $4, $5, $6, $7)',
			[scan, physical, blood, doctor.rows[0].doctor_office, doctor.rows[0].doctor_id, patient.rows[0].patient_id, patient.rows[0].patient_diagnosis]);
...
router.put('/update/diagnosis', doc, async (req, res) => {
	try {
		...
		const diagnosis = await db.query('SELECT * FROM diagnosis WHERE diagnosis_symptoms = $1 AND diagnosis_condition = $2',
			[symptoms, condition]);

		if (diagnosis.rows.length === 0) {
			return res.status(401).json({ message: 'Please enter a valid diagnosis' });
		}
